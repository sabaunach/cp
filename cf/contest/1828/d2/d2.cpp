...
struct Tree {
    ...
    vector<T> s; int n;
    Tree(int n, function<T(T, T)> f, T unit) : unit(unit), s(2*n, unit), n(n), f(f) {}
...
void solve() {
    ...
    auto mnt = Tree(n, [] (int a, int b) { return min(a,b); }, INT_MAX);
    auto mxt = Tree(n, [] (int a, int b) { return max(a,b); }, 0);
    rep(i,0,n) mnt.update(i, v[i]), mxt.update(i, v[i]);

    cout << v << "\n";
    rep(i,0,n) {
        auto k = lstTrue(0,i,[&](const int& x) {
            return mnt.query(x,i) < v[i];
        });
        auto x = lstTrue(0,k,[&](const int& x) {
            return mxt.query(x,k) > v[i];
        });
        auto y = fstTrue(i+1,n,[&](const int& x) {
            return mnt.query(x,n) < v[i];
        });
