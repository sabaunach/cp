...
vector<int> val;
vector<int> res;
vector<pair<set<int>, map<int, int>>> freq_r;


void dfs1_a(int v, pair<set<int>, map<int, int>>& f) {
    if (f.se.count(val[v])) f.se[val[v]]++;
    else if (f.fi.count(val[v])) { f.fi.erase(val[v]); f.se[val[v]] = 1; }
    else f.fi.insert(val[v]);
    fore(c, adj[v]) {
        adj[c].erase(v);
        dfs1_a(c, f);
    }
...
void dfs1_b(int v, pair<set<int>, map<int, int>>& f, pair<set<int>, map<int, int>>& t) {
    res[v] = max(f.se.size() ? prev(f.se.end())->fi : 0, t.se.size() ? prev(t.se.end())->fi : 0);
    int k = val[v];
    if (t.se.count(k)) t.se[k]++;
    else if (t.fi.count(k)) { t.fi.erase(k); t.se[k] = 1; }
    else t.fi.insert(k);
    if (f.se.count(k)) {
        f.se[k]--;
        if (f.se[k] == 1) { f.se.erase(k); f.fi.insert(k); }
        else if (f.se[k] == 0) { f.se.erase(k); }
    } else f.fi.erase(k);
...
void dfs1(int v = 0) {
    freq_r.resize(sz(adj[v]));
    int i = 0;
    fore(c, adj[v]) {
        adj[c].erase(v);
        dfs1_a(c, freq_r[i++]);
    }
    pair<set<int>, map<int, int>> tot;
    fore(x, freq_r) {
        for (auto&[k,f]: x.se) {
            if (tot.se.count(k)) tot.se[k] += f;
            else if (tot.fi.count(k)) { tot.fi.erase(k); tot.se[k] = f + 1; }
            else tot.se[k] = f;
        }
        for (auto& k: x.fi) {
            if (tot.se.count(k)) tot.se[k]++;
            else if (tot.fi.count(k)) { tot.fi.erase(k); tot.se[k] = 2; }
            else tot.fi.insert(k);
        }
    }
    i = 0;
    fore(c, adj[v]) {
        for (auto&[k,f]: freq_r[i].se) {
            tot.se[k] -= f;
            if (tot.se[k] == 1) { tot.se.erase(k); tot.fi.insert(k); }
            else if (tot.se[k] == 0) { tot.se.erase(k); }
        }
        for (auto& k: freq_r[i].fi) {
            if (tot.se.count(k)) {
                tot.se[k]--;
                if (tot.se[k] == 1) { tot.se.erase(k); tot.fi.insert(k); }
                else if (tot.se[k] == 0) { tot.se.erase(k); }
...
void dfs2(int v = 0) {
    ...
    int mx1 = v, mx2 = v;
    fore(c, adj[v]) {
        if (d[c] > (mx1 == v ? r[v] : d[mx1])) {
            mx2 = mx1; mx1 = c;
        } else if (d[c] > (mx2 == v ? r[v] : d[mx2])) {
            mx2 = c;
        }
    }
    int mx_d = 0;
    fore(c, adj[v]) {
        chmax(mx_d, d[c]);
        r[c] = (mx1 == c ? (mx2 == v ? r[v] : d[mx2]) : (mx1 == v ? r[v] : d[mx1])) + 1;
        dfs2(c);
    }
    mx[v] = max(r[v], mx_d);
}
