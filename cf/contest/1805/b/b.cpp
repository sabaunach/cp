...
void solve() {
    // lexographically minimal <-> acb, aacc, abbcb, aaab
    // <-> move min element at last pos to front?
    int n; cin >> n;
    string s; cin >> s;
    map<char, vector<int>> m; rep(i,0,n) m[s[i]].pub(i);
    cout << m.begin()->fi;
    rep(i,0,n) if (i != m.begin()->se.back()) cout << s[i];
    cout << "\n";
}
