...
void solve() {
    int n, m; cin >> n >> m;
    // for each parabola check to see if there is a line that doesn't touch it
    // ax^2 + bx + c
    // line touches <-> (0,0)->(x,y) point tangent to parabola
    // ax^2 + bx + c = kx
    // ax^2 + (b-k)x + c = 0
    // solve for x in terms of k?
    // (k-b) +- sqrt((b-k)^2 - 4*a*c) / 2a
    // => (b-k)^2 - 4*a*c >= 0
    // => |b-k| >= sqrt(4*a*c)
    // => b-k >= sqrt(4*a*c) or b-k <= -sqrt(4*a*c)
    // => k <= b - sqrt(4ac) or k >= b + sqrt(4ac)

    set<ll> lines; rep(i,0,n) { ll k; cin >> k; lines.insert(k); }
    rep(i,0,m) {
        ll a,b,c; cin >> a >> b >> c;
        if (4*a*c < 0) { cout << "NO\n"; continue; }
        ll sqr = sqrt(4*a*c);
        while (sqr*sqr < 4*a*c) sqr++;
        while (sqr*sqr > 4*a*c) sqr--;
        ll l, r;
        if (sqr*sqr == 4*a*c) { l = b - sqr, r = b + sqr; }
        else l = b - ceil(sqrt(4*a*c)), r = b + ceil(sqrt(4*a*c));

        if (lines.upper_bound(l) != lines.end() && *lines.upper_bound(l) < r) { cout << "YES\n" << (ll)*lines.upper_bound(l) << "\n"; continue; }
        cout << "NO\n";
    }
    cout << "\n";
}
