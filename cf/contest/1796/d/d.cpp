...
void solve() {
    ...
    // for positive x:
    // if window size <=k, incr everything in window
    // if window size >k, this is just 2*k*x + max sum of array offset by -x
    //
    // for negative x:
    // if window size <= n-k, incr everything in window
    //  i.e. first incr everything by x, then choose the elements on the sides, to not include
    // if window size > n-k, this is just 2*(n-k)*(-x) + max sum of array offset by x
    ...
    int ans = 0;
    if (x<0) { k=n-k; x=-x; }
        // window size capped by k, everything increased by x

    for (auto &e: v) e += x;
    p[0] = v[0]; for (int i = 1; i < n; i++) p[i] = p[i-1] + v[i];
    list<int> l = {-1};
    auto pv = [p] (int i) { return i==-1 ? 0 : p[i]; };
    for (int i = 0; i < n; i++) {
        while (l.size() && l.front() < i-k)
            l.pop_front();
        ckmax(ans, l.size() ? pv(i) - pv(l.front()) : 0);
        while (l.size() && pv(l.back()) >= pv(i))
            l.pop_back();
        l.push_back(i);
    }
    // window size at least k, sum increased by 2*k*x
    for (auto &e: v) e -= 2*x;
    p[0] = v[0]; for (int i = 1; i < n; i++) p[i] = p[i-1] + v[i];
    // max sum of window ending at i = p[i]-min{p[j]} for j<i-k
    int min_p = 0;
    for (int i = k; i < n; i++) {
        ckmax(ans, p[i] - min_p + 2*x*k);
        ckmin(min_p, p[i-k]);
    }
