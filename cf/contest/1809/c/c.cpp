...
void solve() {
    int n,k; cin >> n >> k;
    int i = fstTrue(0,n+1,[&] (auto &x) { return n*(n+1)/2 - (n-x)*((n-x)+1)/2 >= k; })-1;
    if (i == -1) { rep(j,0,n) cout << -1 << " "; cout << "\n"; return; }
    vector<int> v(n, 30);
    // rep(j, i+1, n) v[j] = -(i+2);
    int r = (i+1)*((i+1)+1)/2;
    int j = i+1, sum = 0;
    while (r < k) {
        if (v[j] > 0) v[j] = -(i+1)*30+sum-1;
        // cout << j << " " << v[j] << "\n";
        v[j]+=30; r++;
        // if (v[j]>=0) v[j] = -1;
        if (v[i]-sum+v[j]>=0) { v[j] = -1; sum -= v[j]; j++; if (j<n) v[j] = -(i+1)*30+sum-1; }
    }
    // cout << "i: " << i << ", r: " << r << ", j: " << j << "\n";
    for (;j < n; j++) if (v[j] > 0) v[j] = -1000;
    fore(e, v) cout << e << " "; cout << "\n";
