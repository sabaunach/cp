...
// all inc except for in one spot?
// 2 3 1 4 5
// 2 3 4 1 5 <- is bad?

// 1 2 4 3, (4,3)
// 2 1 3 4, (2,1)
// sorted, except for in one spot
// that spot either at beginning or end
// so # swaps to get to that near sorted?
// 1 3 2 4 5
...
// 1,2,4 but we'd like to have a cycle of length 2 of adj elements at the end?
// can do the cycles in any order?
// do the cycles in an order s.t. we leave 2 adj elements?
// each swap sorts 1 and leaves 1 unsorted until the last swap?
//

// 3 4 1 2
// 1->3, 3->1, 4->2, 2->4
// so no cycles with adj elements
// so 1+1+1

// if on the way to making it sorted
// there is a way to have the last operation
// be swapping two adjacent elements
// then we can do it in (# op - 1)
// otherwise (# op + 1)
//
...
void solve() {
    ...
    rep(i,0,n-1) if (cyc[i] && cyc[i]==cyc[i+1]) { cout << cnt-1 << endl; return; }
    cout << cnt+1 << endl;
}
