...
void solve() {
    R(n); vii v(n);
    rep(i,0,n) { cin >> v[i].fi; v[i].se = i; }
    sort(all(v));
    vi p(n,-1), q(n,-1);
    set<int> a, b;
    rep(i,1,n+1) { a.insert(a.end(), i); b.insert(b.end(),i); }
    for (auto[x,i]: v) {
        if (a.count(x)) {
            if (*b.begin() <= x) {
                p[i] = x; q[i] = *b.begin();
                a.erase(x); b.erase(b.begin());
                continue;
            }
        }
        if (b.count(x)) {
    ...
    cout << "YES" << endl;
    fore(x, p) cout << x << " "; cout << endl;
    fore(x, q) cout << x << " "; cout << endl;
}
