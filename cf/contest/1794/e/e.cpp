...
ll mod1 = 2e9+33, mod2 = 2e9+1953;
struct Mod {
    ll x=0, y=0;
    Mod() {}
    Mod(ll a) : x(a), y(a) {}
    Mod(ll xx, ll yy) : x(xx), y(yy) {}
    Mod operator+(Mod b) { return Mod((x + b.x) % mod1, (y + b.y) % mod2); }
    Mod operator-(Mod b) { return Mod((x - b.x + mod1) % mod1, (y - b.y + mod2) % mod2); }
    Mod operator*(Mod b) { return Mod((x * b.x) % mod1, (y * b.y) % mod2); }
    bool operator<(const Mod& b) const { return make_pair(x,y) < make_pair(b.x, b.y); }
};
...
set<Mod> pows;
set<int> ans;
Mod dfs1(int x, int p = -1) {
    for (auto c: adj[x]) {
        if (c == p) continue;
        dfs1(c, x);
        h1[x] = (h1[x] + h1[c]*base+Mod(1,1));
    }
    // cout << x+1 << ": " << h1[x].x << " " << h1[x].y << "\n";
    return h1[x];
}

void dfs2(int x, int p = -1) {

    h2[x] = (x ? h2[p]: h1[x]*base+Mod(1));
    h2[x] = h2[x] - (h1[x]*base+Mod(1));
    h2[x] = h2[x]*base + h1[x] + Mod(x!=0);

    // cout << "-> " << x+1 << ": " << h2[x].x << " " << h2[x].y << "\n";
    if (pows.count(h2[x] - target)) ans.insert(x+1);
    for (auto c: adj[x]) {
        if (c == p) continue;
        dfs2(c, x);
    }
