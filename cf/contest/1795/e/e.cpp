...
int N, M, K, Q, X, Y, T;
void solve() {
    // need at least MP of largest monster
    // if we kill largest monster in middle we'd have to
    // kill either of the subgroups on L and R
    // so either we explode biggest monster after recursing L and R,
    // or we bolt down biggest monster and recurse L and R, using exp
    // only on one of them
    // can make tree on monsters?
    // health not unique?
    //

    // find the best one to explode from
    // -> would be the one with
    //  the surrounding subarray
    //  where we'd get the most damage from the explosion
    // -> L subarray strictly increasing, sum of it + extra if we get
    // -> R subarray strictly decreasing, sum of it + extra if we get
    //  we can also modify some other elements to get this by +-1
    // cost of this would be diff from nxt el i.e.
    //  6 3 -> costs 4 to fix
    //
    // actually just computing max amt of value we get from explosion
    // 1: forward, decreasing as needed
    //  4 3 1 2 1, 4 5 3 1 2 1
}
