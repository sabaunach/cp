...
void solve() {
    ...
    // 2 2 2
    // 2 4 2
    // find # colorings with max weight
    // paint n/2 vertices red, n/2 blue
    // weight = sum of edges connecting two verts w/ different colors
    // RBB: 7+8
    // BRB: 7+5
    // BBR: 8+5
    ...
    // 8+5+2+2+3+3+2+4
    // count # colorings:
    // each component should have 1 red, 2 blue or 2 red, 1 blue
    // first choose n/2 components to have 2 red
    // casework:
    //  123: one way to color
    //  113: two ways to color
    //  133: one way to color
    //  111: three ways to color

    // (n/3 choose n/6)*(2 * #113)*(3 * #111)
    // (4 choose 2)*(2)*(3)
    ...
    for (int i = 0; i < n/3; i++) {
        ...
        sort(v.begin(), v.end());
        if (v[0] == v[1] && v[1] == v[2]) three++;
        else if (v[0] == v[1] && v[1] < v[2]) two++;
    }

    cout << (((binom(n/3, n/6)*binpow(2, two, MOD)) % MOD)*binpow(3, three, MOD)) % MOD << "\n";
