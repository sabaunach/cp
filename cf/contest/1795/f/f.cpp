...
// return: is black node here coming up? how long is path?
// going down, how long is path clear?

vector<vector<int>> adj;
vector<int> a, ans;
// x, xmv if send x down, xmv if send x up, open if send x dn)
vector<int> blx, xdn, xup, odn,
tuple<bool, int, int> dfs(int v, int p) {
    if (adj[v].size() == 1 && adj[v][0] == p)
        return {a[v], !a[v], 0};

    vector<tuple<bool, int, int>> ress;
    for (auto c: adj[v]) {
        if (c == p) continue;
        ress.push_back(dfs(c, v));
        if (a[v]) {
            if (a[c]) ans[c] = 0;
        } else {
            // whichever chip stands to gain the most from
            // coming up
            //
        }
    }
