...
void solve() {
    RNV();
    vi a, b; FOR(i,1,n+1) { a.push_back(i); b.push_back(n+1-i); }
    int c1 = 0, c2 = 0, u = 0;
    FOR(i,0,n) {
        if (v[i] != a[i] && v[i] == b[i]) c1++;
        if (v[i] != b[i] && v[i] == a[i]) c2++;
        if (v[i] != a[i] && v[i] != b[i]) u++;
    }
    if (c1 + u <= c2) cout << "First\n";
    else if (c2 + u < c1) cout << "Second\n";
    else cout << "Tie\n";
}
