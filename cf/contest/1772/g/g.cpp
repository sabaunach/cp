...
void solve() {
    R(n); R(x); R(y);
    vi v = vi(n); FORE(z,v) cin >> z;
    sort(ALL(v));

    int i = -1, tot = 0;
    while (true) {

        // Which is the last opponent Monocarp can beat
        while (i+1<n && v[i+1] <= x+(i+1)) i++;
        // Monocarp gets to y on this round
        if (x+i+1>=y) { cout << tot + y - x << "\n"; return; }

        int incr = 2*(i+1)-n;
        // Monocarp can't gain any rating (relatable)
        if (incr <= 0) { cout << "-1\n"; return; }

        // simulate rounds of Monocarp beating until either:
        //  (1) Monocarp hits y on next iteration
        //  (2) Monocarp hits next index on next iteration
        int target = y-i;
        if (i+1<n) UMIN(target, v[i+1]-(i+1));
        int rounds = max((target-x)/incr-1,(int)1);
        x += rounds*incr;
        tot += rounds*n;
    }
}
