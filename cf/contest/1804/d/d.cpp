...
#define tcitT template<class T, typename = typename enable_if<is_iterable<T>::value>::type
...
void solve() {
    // occupied if at least one window is bright
    // each floor has exactly m/4 two-bedroom and m/2 one-bedroom
    // minimize # occupied rooms:
    // put the 01, 10s to two-bedrooms
    // maximize # occupied rooms:
    //  need to put in the two-bedrooms first
    //  10: 2-bed L, 01: 2-bed R, keep propo this?
    //  11...11 or 01...11 or 11...10 or 01...10
    //  now it shouldn't matter?
    // put the 11s to one-bedrooms first?
    ...
    rep(i,0,n) {
        vc v(m); cin >> v;
        vc v2 = v;
        int mno = 0, mxo = 0, mnt = 0, mxt = 0;
        rep(j,0,m-1) {
            if (mnt < m/4 && (v[j]=='1' && v[j+1] == '1')) {
                v[j] = '2'; v[j+1] = '2'; mnt++;
            }
        }
        rep(j,0,m) {
        ...
        rep(j,0,m-2) {
            if (mxo < m/2 && (v[j] == '1' || v[j] == '2') && v[j+1] == '1' && v[j+2]=='1') {
                mxo++; v[j+1] = '2';
            }
        }
        rep(j,0,m-1) {
            if (mxo >= m/2 && mxt < m/4 && v[j] == '1' && v[j+1] == '1') {
                mxt++; v[j] = '2'; v[j+1] = '2';
            }
        }
        int x = 0;
