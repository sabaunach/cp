...
bool solve() {
    int n,x; ll p; cin >> n >> x >> p;
    vl can_reach(n, INT_MAX);
    mod = n;
    Mod m(x);
    rep(i,1,min(n+1,(int)p+1)) { m = m+i; chmin(can_reach[m.v], (ll)i); }
    m = m - x;

    for (int i = 0; i < n; i++) {
        if (can_reach[i] != INT_MAX) {
            int it = 1;
            for (Mod j = m + i; can_reach[i]+it*n <= p && can_reach[j.v] == INT_MAX; j = j + m, it++) {
                can_reach[j.v] = can_reach[i]+it*n;
            }
        }
    }
    return can_reach[0] != INT_MAX;
}
