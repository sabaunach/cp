...
void solve() {
     ...
     * 0: already good
     * 1: can change one element to make good
     * 2: otherwise
     */
    /*
     * c[i] = # tests starting at i if i would be valid, -1 otherwise
     * ways to get 1 op: get to a c[i] from current pos, s.t.
     *  v[i] = c[j] + 1 (1 being we change the el right after i)
     *  we could also change 1 el somewhere else?
     *  v[i] = c[a] + c[b] + 1?
     *  with c[a] ending at some point?
     *
     * d[i] = # tests starting at i if we use 1 op. theoretically many options for # tests
     * 1 is always an option. 2 if we can split into two tests. 3, etc.
     *
     * try making paths
     *
     */
