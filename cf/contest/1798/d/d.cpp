...
void solve() {
    ...
    fore(x, v) chmax(mx, x), chmin(mn, x);
    ll rg = mx-mn;
    // max abs value sum subarray < max-min
    // max abs value sum subarray < range

    // minimize the maximum sum of any subarray
    // must have a negative number

    // max...neg stuff... 2max... negstuff...
    vi pos, neg;
    fore(x, v) if (x >= 0) pos.pub(x); else neg.pub(x);
    ...
    while (i < sz(pos) || j < sz(neg)) {
        if (i < sz(pos) && abs(mxsum + pos[i]) < rg) res.pub(pos[i++]);
        else if (j < sz(neg) && abs(mnsum + neg[j]) < rg) res.pub(neg[j++]);
        else { cout << "No\n"; return; }
        mxsum = max(mxsum + res.back(), 0LL);
        mnsum = min(mnsum + res.back(), 0LL);
    }
    cout << "Yes\n";
    fore(x, res) cout << x << " "; cout << "\n";
}
