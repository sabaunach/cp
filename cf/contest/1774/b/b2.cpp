...
bool solve() {
    R(n); R(m); R(k);
    vi v(m); RV(v);
    // can we do the colors
    // <=> can we arrange such that for every k, colors distinct
    // greedily we'd pick most frequent colors and use them whenever we can
    //
    if (k>m) return false;
    priority_queue<pii, vector<pii>, std::greater<pii>> pq;
    sort(ALL(v));
    FOR(i,0,k) pq.push({v[i], v[i]});
    int tot = 0, j = k;
    while(!pq.empty()) {
        if (j >= m) break;
    ...
    int val = pq.top().se; pq.pop();
    while (!pq.empty()) {
        if (pq.top().se != val) return false;
        pq.pop();
    }
    return true;
}
