...
void solve() {
    R(n);
    vector<vi> ans(n);
    list<int> l; rep(i,1,(n*n+1)/2+1) {
        l.push_back(i);
        l.push_back(n*n+1-i);
    }
    rep(i,0,n) rep(j,0,n) { ans[i].push_back(l.front()); l.pop_front(); }
    rep(i,0,n) if (i%2) reverse(all(ans[i]));
    fore(r, ans) { fore(x, r) cout << x << " "; cout << "\n"; }
}
