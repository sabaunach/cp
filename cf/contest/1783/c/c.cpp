...
void solve() {
    R(n); R(m);
    vi a(n); RV(a);
    vi w(n); rep(i,0,n) w[i]=i+1;
    vii v; rep(i,0,n) v.emplace_back(a[i], -i);
    sort(all(v));
    int x = 0;
    for (;x<n && m-v[x].fi>=0;x++) {
        m-=v[x].fi;
        w[-v[x].se]--;
    }
    // undo last one
    ...
    // see biggest we can win against & tie with
    // we could tie with x+2?
    // but need to keep skip must be x then
    // can't have  x+1,x+2?
    // cout << "m=" << m << ", x=" << x << endl;
    int b = x;
    for (;b<n && m-v[b].fi>=0 && ((w[b]<=x+2 && (b<0 || b == x || w[b-1]<=x)) || w[b]<=x+1);b++);
    if (b>x) {
        if (b==n) b--;
        x++;
        w[-v[b].se]--;
    ...
    // cout << "wins: " << x << endl;
    sort(all(w));
    // fore(y, w) cout << y << " "; cout << "\n";
    auto pos = upper_bound(all(w), x);
    // cout << "pos: " << pos - w.begin() << endl;
    if (pos == w.begin()) {
        cout << n+1 << "\n"; return;
    }
    pos--;
