...
void solve() {
    ll n,m; cin >>n>>m;
    v<vi> adj(n); vi par(n);
    vl a(n); vl sm(n); vi ssz(n);
    v<set<pi>> st(n);
    v<set<pi>::iterator> bck(n);
    cin >> a;
    rep(i,0,n-1) {
        int u,v; cin>>u>>v; u--; v--;
    ...
    dfs = [&] (int v, int p) {
        ...
        fore(c, adj[v]) {
            ...
            dfs(c, v);
            par[c] = v;
            sm[v] += sm[c]; ssz[v] += ssz[c];
            bck[c] = st[v].emplace(-ssz[c], c).first;
        }
    };
    dfs(0, -1);
    ...
    rep(i,0,m) {
        ...
        auto hc = st[x].begin(); if (hc == st[x].end()) continue;
        auto c = hc->second;
        auto p = par[x];
        st[p].erase(bck[x]); sm[p] -= sm[x]; ssz[p] -= ssz[x];
        st[x].erase(hc); sm[x] -= sm[c]; ssz[x] -= ssz[c];

        par[x] = c; sm[c] += sm[x]; ssz[c] += ssz[x];
        bck[x] = st[c].emplace(-ssz[x], x).first;

        par[c] = p; sm[p] += sm[c]; ssz[p] += ssz[c];
        bck[c] = st[p].emplace(-ssz[c], c).first;
    }
}
