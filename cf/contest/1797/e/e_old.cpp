...
struct Eq_Node{
    ...
    Eq_Node(int lo,int hi):lo(lo),hi(hi){} // Large interval of -inf
    Eq_Node(vi& v, int lo, int hi) : lo(lo), hi(hi) {
        if (lo + 1 < hi) {
            int mid = lo + (hi - lo)/2;
            l = new Eq_Node(v, lo, mid); r = new Eq_Node(v, mid, hi);
            val = l->val && r->val && (l->val < 0 || r->val < 0 || l->val == r->val) ? max(l->val, r->val) : 0;
        }
        else val = v[lo];
    }
    int query(int L, int R) {
        ...
        push();
        auto lq = l->query(L, R), rq = r->query(L, R);
        return lq && rq && (lq < 0 || rq < 0 || lq == rq) ? max(lq, rq) : 0;
    }
    ...
    void push() {
        if (!l) {
            int mid = lo + (hi - lo)/2;
            l = new Eq_Node(lo, mid); r = new Eq_Node(mid, hi);
        }
        if (mset != inf)
...
void solve() {
    ...
    int H = 25;
    vi v(n); cin >> v;
    vector<Eq_Node*> st(H+1);
    rep(i,0,H+1) st[i] = new Eq_Node(0, n);
    auto ht = new Sum_Node(0, n);
    map<int, int> mp;
    ...
    auto update = [&] (int i, int x) {
        ...
        for (int lv = ht->query(i, i+1); lv > h; lv--) {
            st[lv]->set(i, i+1, 0);
        }
        ht->set(i, i+1, h);
...
//    rep(i,0,n) cout << ht->query(i,i+1) << " "; cout << "\n\n";
//    rep(hh,0,4) {
//        rep(i,0,n) {
//            cout << st[hh]->query(i,i+1) << " ";
//        } cout << "\n";
//    } cout << "\n";
    ...
    rep(i,0,m) {
        ...
        if (t == 1) {
            ...
            while (it != mp.end() && it->fi < r) {
                auto[x,h] = *it; it++;
                update(x, phi[st[h]->query(x, x+1)]);
            }
//            rep(j,0,n) cout << ht->query(j,j+1) << " "; cout << "\n\n";
//            rep(hh,0,4) {
//                rep(j,0,n) {
//                    cout << st[hh]->query(j,j+1) << " ";
//                } cout << "\n";
//            } cout << "\n";

        } else {
            int h = 0;
            while (st[h]->query(l, r)) h++;
            cout << ht->query(l, r) - (h-1)*(r-l) << "\n";
        }
    }
