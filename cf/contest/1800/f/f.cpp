...
void solve() {
    // combine pairs s.t.
    // # different letters = 25,
    // # each occurrence of each letter is odd
    //  => either even in A, odd in B or even in B, odd in A
    // the 00 must be not in either word

    int n; cin >> n;
    vector<map<int, ll>> vm(26);
    vector<int> cnt(1<<26);
    for (int i = 0; i < n; i++) {
        string s; cin >> s;
        int vc = 0, x = 0;
