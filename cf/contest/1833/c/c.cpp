...
bool solve() {
    int n; cin >> n;
    vi a(n); cin >> a;
    /* smallest odd subtract from all evens or smallest even subtract from all odds */
    int sme = INT_MAX, smo = INT_MAX;
    fore(x, a) if (x % 2) chmin(smo, x); else chmin(sme, x);
    if (sme == INT_MAX || smo == INT_MAX) return true;
    if (sme <= smo) return false;
    return true;
}
