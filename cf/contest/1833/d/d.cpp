...
void solve() {
    int n; cin >> n;
    vi v(n); cin >>v;
    int r = 1; rep(i,1,n) if (v[i] > v[r]) r = i;
    vi bst;
    if (r == n-1) {
        rep(l,0,r+1) {
            vi cur(v.begin() + l, v.end());
            reverse(all(cur));
            rep(i,0,l) cur.pub(v[i]);
            if (bst.empty() || cur > bst) bst = cur;
        }
    } else {
        rep(l,0,r) {
            vi cur(v.begin() + r, v.end());
            rep(i,l,r) cur.pub(v[i]);
            reverse(cur.begin()+(n-r), cur.end());
            rep(i,0,l) cur.pub(v[i]);
            if (bst.empty() || cur > bst) bst = cur;
        }
    }
    fore(x, bst) cout << x << " "; cout << "\n";
}
