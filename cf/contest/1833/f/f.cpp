...
void solve() {
    /*
    # of subsequences of m students
    pairwise distinct with all elts |diff| < m
    2 2 3 4 6
    m=3
    i.e. they have to be inc by 1 subseq of length m
    ...
    */
    ...
    auto it = mp.begin();
    for (; it != mp.end(); it++) {
        if (l.size() && it->fi - l.back().fi > 1) {
            l.clear(); sz = 0; prod = 1;
        }
        sz++; prod = prod * it->se; l.push_back(*it);
        if (sz > m) { prod = prod / l.front().se; l.pop_front(); sz--; }
        if (sz == m) cnt = cnt + prod;
    }
    cout << cnt.v << "\n";
}
