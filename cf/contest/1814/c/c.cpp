...
void solve() {
    ...
    vpi r(n); rep(i,0,n) { cin >> r[i].fi; r[i].se = i+1; }
    sort(all(r)); reverse(all(r));
    // want to minimize
    // s1*r[v1[0]]+2*s1*r[v1[1]]+3*s1*r[v1[2]]+...
    // + s2*r[v2[0]]+2*s2*r[v2[1]]+3*s2*r[v2[2]]+...
    // more requested balls at the front
    // should be greedy
    vi a,b; ll t_a = 1, t_b = 1;
    rep(i,0,n) {
        if (s1*t_a*r[i].fi < s2*t_b*r[i].fi) {
            a.pub(r[i].se);
            t_a += 1;
        } else {
            b.pub(r[i].se);
