...
void solve() {
    ...
    int ans = INT_MAX;
    rep(i,1,100000) chmin(ans, i - 1 + a/i + (bool)(a%i) + b/i + (bool)(b%i));
    cout << ans << "\n";

//    int k = 1000;
//    int c = lstTrue(1, b+1, [a,b,k] (auto& c) {
//        return c + a/c + (bool)(a%c) + b/c + (bool)(b%c)
//        >= (c+k) + a/(c+k) + (bool)(a%(c+k)) + b/(c+k) + (bool)(b%(c+k));
//    });
//    int ans = INT_MAX;
//    rep(i,max(1, c-k), c+k) {
//        chmin(ans, i - 1 + a/i + (bool)(a%i) + b/i + (bool)(b%i));
//    }
//    cout << ans << "\n";
...
//    int a = 12345, b = 6969;
//    rep(c,1,b) {
//        cout << c-1 + a/c + b/c + (bool)(a%c) + (bool)(b%c) << " ";
//    }
