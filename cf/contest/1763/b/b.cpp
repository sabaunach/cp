...
bool solve() {
    ...
    RV(h); RV(p);
    multiset<int> power;
    multimap<int, multiset<int>::iterator> health;
    FOR(i,0,n) {
        health.insert({h[i], power.insert(p[i])});
    }
    ...
    while (true) {
        d+=k;
        while(it != health.end() && it->first<=d) { power.erase(it->second); it++; }
        if (it==health.end()) return true;
        k-=*power.begin();
        if (k<=0) return false;
    }
}
