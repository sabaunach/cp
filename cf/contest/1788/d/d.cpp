...
void solve() {
    cin >> N;
    vi v(N); cin >> v;
    vector<map<ll, ll>> vm_ll(N), vm_rl(N);
    vl tot(N), tll(N), trl(N);
    ...
    FOR(i,0,N) {
        ...
        map<ll, ll> llm, rlm;
        FOR(j,0,i) {
            ll cll = prev(vm_ll[j].ub(v[i]-v[j]))->second;
            ll crl = prev(vm_rl[j].ub(v[i]-v[j]))->second;
            ll dll = tll[j] - cll;
            ll drl = trl[j] - crl;
            (tot[i] += dll) %= MOD;
            rlm[v[i]-v[j]] = (dll+drl+1)%MOD;
            if (cll+crl>0) llm[v[i]-v[j]] = (cll+crl)%MOD;
        }
        vm_ll[i][-1e9] = 0; vm_rl[i][-1e9] = 0;
        ll sum = 0;
        for (auto&[k,v]: llm) sum = vm_ll[i][k] = (sum + v) % MOD;
        sum = 0;
        for (auto&[k,v]: rlm) sum = vm_rl[i][k] = (sum + v) % MOD;
        tll[i] = (vm_ll[i].rbegin())->second;
        trl[i] = (vm_rl[i].rbegin())->second;
        tot[i] += tll[i] + trl[i];
    }
//    FOR(i,0,N) {
//        cout << v[i] << ": \n\tvm_ll: ";
//        for (auto &[k,v]: vm_ll[i]) cout << "{" << k << ", " << v << "}, ";
//        cout << "\n\tvm_rl: ";
//        for (auto &[k,v]: vm_rl[i]) cout << "{" << k << ", " << v << "}, ";
//        cout << "\n";
//    }
    ll tot_tot = 0;
    each(x, tot) (tot_tot += x) %= MOD;
    cout << tot_tot << "\n";
}
